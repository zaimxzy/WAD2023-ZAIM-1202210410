...
function login($input) {
    ...
    if(mysqli_num_rows($result) == 1) {
        ...
        if(password_verify($password, $data['password'])) {
            ...
            if (isset($input['remember'])){
                setcookie('id', $data['id'], time() + 3600, "/WAD2023-ZAIM-1202210410/MODUL4%20ZAIM/views");
            }
            // 
    ...
    // (5) Buat kondisi else, kemudian di dalamnya buat variabel session dengan key message untuk menampilkan pesan error ketika email tidak ditemukan
        } else {
